...
async function mailOrder(){
  ...
  try{
    ...
    toast('Sipariş Excel eki ile mail olarak gönderildi.');
  }catch(e){ toast(e.message || 'Mail gönderilemedi.'); }
  finally{ btn.disabled=false; btn.textContent='Mail Gönder'; }
}
function logout(){
  state.order.clear();
...
document.addEventListener('click', e => { const b=e.target.closest('button'); if(!b) return; if(b.id==='saveBtn') saveOrder(); if(b.id==='mailBtn') mailOrder(); if(b.id==='logoutBtn') logout(); if(b.dataset.act){ const code=b.dataset.code; const current=state.order.get(code)||0; setQty(code, b.dataset.act==='inc'?current+1:current-1); const input=document.querySelector(`.qty input[data-code="${CSS.escape(code)}"]`); if(input) input.value=state.order.get(code)||0; } });
